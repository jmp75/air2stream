...
def _check_valid_options(value: str, valid_options):
    if value not in valid_options:
        raise ValueError(f"Invalid option {value}. Valid options are {valid_options}")


def set_fun_obj(value: str):
    """objective function: KGE, NSE, RMS."""
...
def set_mod_num(value: str):
    ...
    _check_valid_options(value, ["RK4", "EUL", "RK2", "CRN"])
    _cc.mod_num = value


def set_runmode(value: str):
    """optimization algorithm: PSO or LATHYP"""
    _check_valid_options(value, ["PSO", "LATHYP"])
    _cc.runmode = value


_SOME_DEFAUT_PARAMETERS = np.array(
    [0.958264, 0.522815, 0.441700, -0.207825, 18.512997, 5.366099, 0.529306, 3.053014]
)
...
def set_inputs(t_mean_series: pd.Series, streamflow_series: pd.Series):
    ...
    _cc.tt = np.array(
        [x.timetuple().tm_yday / days_in_year(x.year) for x in streamflow_series.index],
        dtype=float,
    )
...
def set_calib_inputs(
    t_mean_series: pd.Series, streamflow_series: pd.Series, water_temp_series: pd.Series
):
...
def use_streamflow_stats(streamflow_not_gapfilled: pd.Series):
    ...
    if len(q_positive) > 0:
        q_media = (
            q_positive.mean()
        )  # code says media (median?) but actually calcualtes mean.
    else:
        q_media = 0
    _cc.qmedia = q_media
